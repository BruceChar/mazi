...
> **状态（2026-09-06，计划 v3）**：以下“已完成”项均已实现并提交；当前质量门禁为 `pnpm check`（36 个测试文件 / 184 个测试）。本文件下部的 F1–F16 单任务 checkbox 是历史 MVP 原始拆解，不再逐项维护。
...
- [ ] core：`GoalContract.loopMode` 类型与默认值；
- [ ] runtime/API：`goal.loopMode` 覆盖透传并持久化；
- [ ] FullLoop：按 mode 决定是否调用 `planner.plan` / `reflector.reflect`；
- [ ] WebUI：新会话/输入可切换 Loop 模式，默认 GPER；
- [ ] 验收：三档模式各自成功/失败路径单测 + `pnpm check`。
