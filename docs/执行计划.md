...
> **状态（2026-09-07，计划 v3）**：以下“已完成”项均已实现并提交；当前质量门禁为 `pnpm check`（36 个测试文件 / 188 个测试）。本文件下部的 F1–F16 单任务 checkbox 是历史 MVP 原始拆解，不再逐项维护。
...
| WebUI Conversation 列表/工作区/归档/多 Session 归属/空状态 | ✅ 已完成 | Vite build + WebUI 相关测试 |
| WebUI 主区连续对话流（移除 Session 导航/id 状态整栏，Conversation 多 Session 自然续排） | ✅ 已完成 | Vite build + `flow.test.ts` |
| Loop 模式（GPER 默认 / GPE / React-only，WebUI 可选） | ✅ 已完成 | strategy/harness-runtime/API/WebUI 测试 |
| 用户原始输入注入每个 LLM 轮次 messages（修复模型看不到输入） | ✅ 已完成 | executor/context-builder 回归测试 + 真实对话验证 |
...
| 问题 | 根因 | 修复 | 验证 |
| --- | --- | --- | --- |
| 真实厂商会话全部失败（单轮/多轮均无法完成） | ① `providers.json`/CLI 预设仍使用已退役模型 `deepseek-chat`，pi-ai 目录不再包含该模型，`resolveModel()` 抛 `未找到模型`；② 工具名 `fs.read` 含 `.`，不满足 OpenAI/DeepSeek `function.name` 的 `^[a-zA-Z0-9_-]+$`，接口直接返回 400；③ mapper 重建工具轮历史时跳过 assistant tool_calls，导致下一轮 `role:'tool' 没有前置 tool_calls` 报错；④ tool 结果消息未携带 `toolCallId`，与 assistant tool_calls 无法对应；⑤ 普通会话仍被注入文件工具并被当作任务执行，简单问候也会反复读目录直至 max-steps | ① DeepSeek 预设与运行配置改用 pi-ai v4 目录模型：`deepseek-v4-flash`（默认）/`deepseek-v4-flash-vision-exp`/`deepseek-v4-pro`；② 防腐层把工具名清洗为 `fs_read` 再发厂商，收到 tool-call 时还原为 `fs.read`；③ LLMMessage 携带工具参数，assistant 工具意图在 mapper 中还原为 `toolCall` 消息；④ 上下文重建时 tool 结果沿用同轮 tool_call 的 `callId`；⑤ HarnessRuntime 应用 Session 级 goal 覆盖，普通会话由 API 注入 `allowedTools=[]` 并切到对话式系统提示词，仅工作区会话保留工具与任务提示词 | 回归测试断言目录内可解析、工具名清洗/还原、toolCall 历史还原与 callId 关联、goal 覆盖与对话提示词生效 + 真实单轮/多轮对话跑通 |

### 下一步计划

#### 0. Loop 模式（多档策略编排，默认 GPER）

目标：Session 级可选执行编排模式，默认按 `Goal → Plan → Execute → Reflect`（GPER）执行。

| 模式 | 编排 | 说明 |
| --- | --- | --- |
| `goal-plan-execute-reflect`（默认） | Goal → Plan → Execute → Reflect | 完整闭环：Planner 分解 → Executor 执行 → Reflector 机械验收 |
| `goal-plan-execute` | Goal → Plan → Execute | 跳过 Reflector；执行成功即接受，用于低风险线性任务 |
| `react-only` | Goal → Execute（单 Turn） | 跳过 Plan/Reflect/文件工具，直接回答，用于实时问答与普通聊天 |

实施要点：

- [x] core：`GoalContract.loopMode` 类型与默认值；
- [x] runtime/API：`goal.loopMode` 覆盖透传并持久化；
- [x] FullLoop：按 mode 决定是否调用 `planner.plan` / `reflector.reflect`；
- [x] WebUI：新会话/输入可切换 Loop 模式，默认 GPER；
- [x] 验收：三档模式各自成功/失败路径单测 + `pnpm check`。

补充（执行语义修正）：
- `react-only` 在 API 创建时强制 `allowedTools=[]`，避免“how to think”这类问题被当成文件任务；
- 新建带 `workspace/workspacePath` 的 Session 会把 runtime 工作区根同步为该路径，而不仅是写 Conversation 元数据；
- 默认 Agent 提示词改为“仅当用户明确要求读/查/改文件或操作工作区时才调用工具，普通问题直接回答”。

执行链路根因修复（2026-09-07）：
- `Executor.buildContext` 此前只把用户输入计入上下文计量段，**没有写入发往模型的 `messages`**，导致模型看不到“how to think”而只能自行读文件/猜测；
- 修复：每个模型轮次的上下文均以用户原始输入作为首条 user 消息注入，历史 Turn/Step 紧随其后；新增回归测试。
...
- Conversation 跨 Session 上下文注入：当前续聊 = 同 Conversation 追加 Session 并独立执行；尚未把前序 Session 摘要/消息注入新 Session，属于多轮语义补全的独立 backlog；
