...
| 问题 | 根因 | 修复 | 验证 |
| --- | --- | --- | --- |
| 真实厂商会话全部失败（单轮/多轮均无法完成） | ① `providers.json`/CLI 预设仍使用已退役模型 `deepseek-chat`，pi-ai 目录不再包含该模型，`resolveModel()` 抛 `未找到模型`；② 工具名 `fs.read` 含 `.`，不满足 OpenAI/DeepSeek `function.name` 的 `^[a-zA-Z0-9_-]+$`，接口直接返回 400 | ① DeepSeek 预设与运行配置改用 pi-ai v4 目录模型：`deepseek-v4-flash`（默认）/`deepseek-v4-flash-vision-exp`/`deepseek-v4-pro`；② 防腐层把工具名清洗为 `fs_read` 再发厂商，收到 tool-call 时还原为 `fs.read` | 回归测试断言目录内可解析 + 工具名清洗/还原 + 真实单轮/多轮对话跑通 |
