...
- **三级权限配置（后端权威）**：① **系统默认** 在 Settings → General，持久化在 `settings.json` 的 `goal.permissionCeiling`（`GET /api/config` / `POST /api/config/goal`）；② **工作区覆盖** `permissions["workspace:<path>"]`；③ **会话覆盖** `permissions["conversation:<id>"]`。②③ 由输入框左下角写入 `POST /api/permissions`（`{scope, key, permissionCeiling}`），二者**相互独立、各自持久化**，一个项目/会话的改动不影响其他；
- **解析**：`resolveScopedPermission(conversation → workspace → 默认)`（`config-io.ts`，服务端在 `createSession` 调用）。输入框只显示/写入当前上下文（有活动会话则写会话，否则写工作区）；客户端若随请求带了 `goal.permissionCeiling` 则优先用于本次（避免异步写入竞态）；
- 已在跑/已建的会话在服务端按 Goal 钉版，后续改默认或改其他作用域都不受影响；`text/draft/approved` 后端仍支持但 UI 只暴露 3 档；
