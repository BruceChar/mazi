...
- 旧的 `workspaces.json` 项目会话关联在首次读取时回填为 Conversation 并清除；`workspaces.json` 仅保留项目 `title/path` 元数据。
- 向已有 Conversation 追加 Session 的入口（续聊）已支持：请求体带 `conversationId` 即追加，WebUI 在当前会话内续聊。
