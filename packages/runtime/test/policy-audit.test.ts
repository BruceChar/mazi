...
describe('RuntimePolicyAuditSink', () => {
    it('condenses all stages of one invocation into a single policy.check', () => {
        ...
        for (const stage of ['supply-check', 'scope-check', 'risk-check', 'approval'] as const) {
    ...
    it('emits policy.denied immediately with the reject code', () => {
        ...
        sink.log(
            event('risk-check', 'denied', {
                code: 'FORBIDDEN_BY_HARD_FLOOR',
                detail: 'nope',
            }),
        );
        expect(events).toHaveLength(1);
        expect(events[0].type).toBe('policy.denied');
        expect((events[0].payload as { code: string }).code).toBe('FORBIDDEN_BY_HARD_FLOOR');
    });
