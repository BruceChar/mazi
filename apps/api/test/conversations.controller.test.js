...
import { createTestApp } from '../src/testing/test-app.js';
describe('conversations（Goal run 会话业务抽象列表）', () => {
    ...
    it('POST /api/sessions 后生成一个含该 Goal run 的 Conversation，并携带用户/工作区归属', async () => {
        ...
        const conversation = conversations.find((item) => item.runs.some((run) => run.rootGoalId === sessionId));
    ...
    it('POST /api/sessions 携带 conversationId 时追加到已有 Conversation', async () => {
        ...
        expect(second.json().conversationId).toBe(firstBody.conversationId);
        const list = (await fastify.inject({ method: 'GET', url: '/api/conversations' })).json();
        const conversation = list.find((item) => item.conversationId === firstBody.conversationId);
    ...
    it('GET /api/conversations 支持 q 筛选（标题/输入）与 limit 分页', async () => {
        ...
        expect(filtered.every((item) => item.title.includes(marker) || item.runs.some((r) => r.input.includes(marker)))).toBe(true);
        const paged = (await fastify.inject({ method: 'GET', url: '/api/conversations?limit=1' })).json();
    ...
    it('PATCH /api/conversations/:id 重命名并归档，DELETE 后级联移除 Goal 树', async () => {
        ...
        const conversation = before.find((item) => item.runs.some((run) => run.rootGoalId === sessionId));
        expect(conversation).toBeDefined();
        ...
        const renamed = after.find((item) => item.conversationId === conversation.conversationId);
        ...
        const finalList = (await fastify.inject({ method: 'GET', url: '/api/conversations' })).json();
        expect(finalList.some((item) => item.conversationId === conversation.conversationId)).toBe(false);
    ...
    it('PATCH /api/workspaces/project 重命名项目展示名', async () => {
        writeFileSync(join(handle.home, 'workspaces.json'), JSON.stringify({
    ...
    it('旧版 conversations.json（sessionIds 结构）不拖垮列表，且可正常新建会话', async () => {
        writeFileSync(join(handle.home, 'conversations.json'), JSON.stringify({
            conversations: [
                {
